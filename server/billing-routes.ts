  ...
  try {
    ...
    if (provider === 'stripe') {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: 'subscription',
        success_url: `${process.env.NODE_ENV === 'development' ? 'http://localhost:5000' : 'https://your-domain.com'}/admin/settings?tab=plans-features&checkout=success`,
        cancel_url: `${process.env.NODE_ENV === 'development' ? 'http://localhost:5000' : 'https://your-domain.com'}/admin/settings?tab=plans-features&checkout=cancelled`,
        metadata: {
          tenantId: currentUser.tenantId,
          planId: planId,
        },
