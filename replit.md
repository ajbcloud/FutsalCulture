...
- **Authentication**: Clerk-hosted authentication (@clerk/clerk-react, @clerk/express) as sole identity provider with Clerk-only enforcement (no legacy session fallback). Users table includes `clerkUserId` field to link Clerk accounts to internal users. Club owners sign up at /club-signup → auto-creates tenant with subscription/plan → redirects to /dashboard (first user gets isAdmin=true). Parents/players join via /join with invite code. Super-Admin role can ONLY be set directly in database (updateUserSchema blocks isSuperAdmin). Tenant creation uses retry logic with increasing entropy for concurrent safety. Admin routes use Clerk-only `requireAdmin` middleware - if Clerk session expires, users see "Session expired. Please sign in again." message.
