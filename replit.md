...
- **Multi-tenant Data Model**: 
  - `users.tenantId` = Which tenant the user belongs to (membership existence, source of truth for access)
  - `tenant_memberships` = Role/tier within that tenant (parent/player/coach/admin + status)
  - Consumer join flow (`/api/consumer/join`) atomically sets both in a transaction
