...
- **Communication System**: Complete template-based email/SMS notification system with automated triggers and manual bulk sending. Includes template management UI, variable replacement ({{parentName}}, {{playerName}}, etc.), notification history tracking, consent management (TCPA compliance), and integration with SendGrid/Twilio. Automated notifications for booking confirmations and session cancellations. Default templates provided for all tenants with admin customization capabilities.
